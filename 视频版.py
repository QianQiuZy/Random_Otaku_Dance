...
def suiwujianji():
    ...
    if len(countdown_files) != 1:
        print(f"倒计时文件夹中包含 {len(countdown_files)} 个文件，请确保该文件夹中仅包含一个倒计时视频文件")
        os.system("pause")
        return
    ...
    # 使用ffmpeg进行视频剪辑，不重新编码
    try:
        (
            ffmpeg
            .input('concat:' + '|'.join(clips), format='concat', safe=0)
            .output(output_file_name, c='copy')  # 使用 copy 选项进行无重新编码的合并
            .run(quiet=True)
        )
